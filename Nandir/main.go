...
func GoDirScan(url string, filename string, num int, errtime int64, timesleep int64, openerrstr int, errstr string) {
	dicall := Readfile(filename)
	r, err := Goscan(url, "", errtime, 0)
	if err != nil {
	...
	for i := 0; i < len(dicall); i++ {
		...
		task := dicall[i]
		g.Run(func() {
			respBody, err := Goscan(url, task, errtime, timesleep)
			if err != nil {
...
func main() {
	...
	if *url != "" && *filename != "" && *openerrstr == 0 {
		GoDirScan(*url, *filename, *xiancheng, *errtime, *timesleep, *openerrstr, "@#!#asddddddddd122222222222asd")
	} else if *url != "" && *filename != "" && *openerrstr == 1 && *errstr != "" {
		GoDirScan(*url, *filename, *xiancheng, *errtime, *timesleep, *openerrstr, *errstr)
	} else {
		flag.Usage()
	}
}
